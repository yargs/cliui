...
// These are the full tests originally used only for cjs, now also run from esm.
// (Not attempting to run from deno.)

require('chai').should()

// stripAnsi adapted from:
// The npm application
...
function runTests (cliui, moduleType) {
  describe(moduleType, () => {
